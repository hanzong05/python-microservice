...
FIX 13 — CSR/CRR now computed using validation-sheet methodology:
          ...
          (C) CRR uses full Robertson-Wride formula with NO cap at N160cs=30
...
class GeotechnicalPipeline:
    ...
    def pre_clean_dataframe(self, df: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
        ...
        spt_raw_col = next((c for c in [
                           'SPT N-Value', 'SPT N Value', 'N-Value', 'SPT'] if c in df.columns), None)
        df['spt_is_imputed'] = (pd.to_numeric(df[spt_raw_col], errors='coerce').isna(
        ...
        if 'Fines Content' in df.columns:
            ...
            if bad.any():
                ...
                self.validation_errors.append(
                    f"[{sheet_name}] Fines >100%: {bad.sum()} rows auto-corrected")
                print(
    ...
    def validate_coordinates(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if 'Latitude' in df.columns:
            ...
            if oor:
                self.validation_errors.append(
                    f"Latitude outside Tarlac range: {oor}")
        if 'Longitude' in df.columns:
            ...
            if oor:
                self.validation_errors.append(
                    f"Longitude outside Tarlac range: {oor}")
        return df

    def validate_spt(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()
        spt_col = next((c for c in ['SPT N-Value', 'SPT N Value',
                       'spt_n_value', 'N-Value', 'SPT'] if c in df.columns), None)
        if spt_col:
            ...
            if oor:
                self.validation_errors.append(
                    f"SPT out of range: {oor} records")
    ...
    def validate_soil_parameters(self, df: pd.DataFrame, sheet_name: str = '') -> pd.DataFrame:
        ...
        uw_col = next((c for c in ['Unit Weight (γ)', 'Unit Weight',
                      'unit_weight', 'Unit Weight (kN/m³)', 'γ'] if c in df.columns), None)
        if uw_col:
        ...
        fc_col = next((c for c in [
                      'Fines Content', 'fines_content', 'Fines (%)'] if c in df.columns), None)
        if fc_col:
        ...
        gwl_col = next((c for c in ['Groundwater Level (m)', 'Ground Water Table',
                       'Groundwater Level', 'groundwater_depth_m', 'GWL'] if c in df.columns), None)
        ...
        fa_col = next((c for c in ['Internal Friction Angle',
                      'Friction Angle', 'friction_angle'] if c in df.columns), None)
        if fa_col:
    ...
    def compute_missing_raw_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if rd_col in df.columns and n160_col in df.columns:
            ...
            ) & n1_60.notna() & (n1_60 > 0) & ~uscs.isin(_COHESIVE_USCS))
            df.loc[miss, rd_col] = (
                np.sqrt(n1_60[miss] / 60.0) * 100.0).round(2)
            if miss.sum():
                print(f"      [FILL] Relative Density: {miss.sum()} rows")
        ...
        es_col = next((c for c in [
                      'Elastic Modulus (Es) (MN/m²)', 'elastic_modulus_es'] if c in df.columns), None)
        if es_col:
    ...
    def fill_missing_by_borehole(self) -> bool:
        ...
        if fault_col:
            ...
            ok = R.notna()
            df.loc[still_default & ok.reindex(
                df.index, fill_value=False), 'pga_g'] = pga_est[ok].round(3).values
            print(
        ...
        if 'spt_is_imputed' in df.columns:
            for bh_id, grp in df.groupby(bh_col, sort=False):
                ...
                if real.sum() >= 2:
                    ...
                    spt_interp = spt_real.interpolate(
                        method='index').ffill().bfill().clip(1.0, 100.0)
    ...
    def calculate_csr_crr(self) -> bool:
        ...
        if bh_col_local:
            df = df.sort_values(
                [bh_col_local, 'depth_from_m']).reset_index(drop=True)
            h_layer = (df['depth_to_m'] - df['depth_from_m']).clip(lower=0.01)
            cum_sv = (
        ...
        # FIX 13B — Use z = depth_to for rd and CSR (not z_mid)
        z = df['depth_to_m']
        depth_below_wt = np.maximum(0.0, z - df['groundwater_depth_m'])
        ...
        print("  Calculating CSR (Seed & Idriss 1971, z=depth_to — FIX 13B)...")
        df['csr'] = 0.65 * df['pga_g'] * \
            (df['total_overburden_pressure'] /
             df['effective_overburden_pressure']) * rd

        # FIX 2 — Cn-corrected N1(60) using effective stress at depth_to
        print("  Computing N1(60) with Cn correction (FIX 2)...")
        raw_n160 = pd.to_numeric(df.get(
            'Corrected SPT-N Value (N1(60))', pd.Series(np.nan, index=df.index)), errors='coerce')
        ...
        print(
            f"    N1(60) source — measured: {has_measured_n160.sum()}, Cn-computed: {(~has_measured_n160).sum()}")

        print("  Computing (N1)60cs with fines correction (NCEER)...")
        FC = df['fines_content'].clip(lower=0.1)
        alpha = np.where(FC < 5.0, 0.0, np.where(
            FC <= 35.0, np.exp(1.76 - 190.0 / FC ** 2), 5.0))
        beta = np.where(FC < 5.0, 1.0, np.where(
            FC <= 35.0, 0.99 + FC ** 1.5 / 1000.0, 1.2))
        df['n1_60cs'] = (alpha + beta * df['spt_n160']
                         )  # no upper clip — FIX 13C

        # FIX 13C — CRR full Robertson-Wride, NO cap at 0.6
        # Dense layers (N160cs>30) get CRR>>0.6 -> FS>>1.5 -> NOT LIQUEFIABLE (correct)
        print("  Calculating CRR (Robertson-Wride, no cap — FIX 13C)...")
        N_crr = df['n1_60cs']
        crr_raw = np.exp(N_crr / 14.1 + (N_crr / 126.0) ** 2 -
                         (N_crr / 23.6) ** 3 + (N_crr / 25.4) ** 4 - 2.67)
        df['crr'] = crr_raw.clip(lower=0.0)  # no upper cap
        ...
        try:
            ...
            try:
                ...
                if old:
                    ...
                    self.client.storage.from_(bucket).upload(
                        f"old_cleaned_data/Cleaned_data_{ts}.csv", old, file_options={'content-type': 'text/csv', 'upsert': 'true'})
                    self.client.storage.from_(bucket).remove([path])
                    print("  [OK] Old file archived")
            except:
                pass
            self.client.storage.from_(bucket).upload(path, df_export.to_csv(index=False).encode(
                'utf-8'), file_options={'content-type': 'text/csv', 'upsert': 'true'})
            print(
            ...
            try:
                ...
                if res.data:
                    ...
                    res = self.client.table('municipalities').insert(
                        {'name': name, 'description': 'Municipality in Tarlac Province'}).execute()
                    self.municipality_ids[name] = res.data[0]['id']
    ...
    def store_soil_layers(self) -> bool:
        ...
        if skipped:
            print(f"    [INFO] Skipped {skipped} records")
        if failed:
        ...
        if not total:
            ...
            return False
        print(
            f"    [OK] Inserted {total} soil layer records (FIX 13 CSR/CRR applied)")
        return True
    ...
    def print_validation_report(self):
        ...
        if self.validation_errors:
            print("\n  [ERRORS]:")
            [print(f"    x {e}") for e in summarise(self.validation_errors)]
        else:
            print("\n  No critical errors")
        if self.validation_warnings:
            print("\n  [WARNINGS]:")
            [print(f"    ! {w}") for w in summarise(self.validation_warnings)]
        else:
            print("\n  No warnings")
    ...
    def run(self) -> bool:
        ...
        for name, fn in steps:
            ...
            if not fn():
                if name == "Export CSV":
                    print("  [INFO] CSV export skipped (no DB connection)")
                else:
...
def main():
    ...
    key = os.getenv('SUPABASE_SERVICE_ROLE_KEY')
    if url and key:
        print("[INFO] Supabase configured — DB storage enabled")
    else:
