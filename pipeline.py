...
# ===========================================================================
# Foundation dimension helpers  (mirrors ANN training target formulas)
# ===========================================================================

def _compute_foundation_B(n: float, fc: float, gwl: float) -> float:
    """
    ANN-consistent B formula (trainer FIX A).
    B = 4.5 / sqrt(N) + fines_offset + gwl_offset, clipped [1.0, 5.0] m.
    """
    ...
    return round(B * 4) / 4   # nearest 0.25 m


def _compute_foundation_D(n: float, fc: float, gwl: float,
                          uscs: str, risk_level: str) -> float:
    """
    ANN-consistent D formula (trainer FIX B).
    D = 1.0 + 0.04*(15-N) + risk_adj + cohesive_adj + gwl_adj, clipped [1.0, 3.5] m.
    """
    n = max(1.0, float(n) if not np.isnan(n) else 15.0)
    fc = float(fc) if not np.isnan(fc) else 15.0
    gwl = float(gwl) if not np.isnan(gwl) else 5.0
    ...
    if uscs in _COHESIVE_USCS or fc >= 35.0:
        D += 0.25
    ...
    return round(D * 4) / 4   # nearest 0.25 m
...
class GeotechnicalPipeline:
    ...
    def pre_clean_dataframe(self, df: pd.DataFrame, sheet_name: str) -> pd.DataFrame:
        ...
        # FIX 8 — GWL backup from 'Ground Water Table'
        if 'Ground Water Table' in df.columns and 'Groundwater Level (m)' in df.columns:
            ...
            if filled.any():
                ...
                print(
                    f"    [FIX8] GWL filled from 'Ground Water Table': {filled.sum()} rows")

        # FIX 9 — track imputed SPT BEFORE filling
        spt_raw_col = next(
            (c for c in ['SPT N-Value', 'SPT N Value', 'N-Value', 'SPT'] if c in df.columns), None)
        df['spt_is_imputed'] = (
            pd.to_numeric(df[spt_raw_col], errors='coerce').isna()
            if spt_raw_col else pd.Series(True, index=df.index)
        )
    ...
    def validate_spt(self, df: pd.DataFrame) -> pd.DataFrame:
        df = df.copy()
        spt_col = next(
            (c for c in ['SPT N-Value', 'SPT N Value', 'spt_n_value', 'N-Value', 'SPT']
    ...
    def validate_soil_parameters(self, df: pd.DataFrame, sheet_name: str = '') -> pd.DataFrame:
        ...
        # Unit weight
        uw_col = next(
            (c for c in ['Unit Weight (γ)', 'Unit Weight', 'unit_weight',
        ...
        # Fines content
        fc_col = next(
            (c for c in ['Fines Content', 'fines_content', 'Fines (%)'] if c in df.columns), None)
        if fc_col:
        ...
        # Groundwater level
        gwl_col = next(
            (c for c in ['Groundwater Level (m)', 'Ground Water Table',
                         'Groundwater Level', 'groundwater_depth_m', 'GWL']
             if c in df.columns), None)
        gwl_numeric = (
            pd.to_numeric(df[gwl_col], errors='coerce')
            if gwl_col else pd.Series(np.nan, index=df.index)
        )
        ...
        # Soil description
        desc_col = next(
            (c for c in ['Soil/Rock Description', 'Soil Description',
                         'soil_description', 'Description'] if c in df.columns), None)
        if desc_col:
            df['soil_description'] = (
                df[desc_col].fillna('').astype(str).str.strip().replace('nan', ''))

        # Friction angle
        fa_col = next(
            (c for c in ['Internal Friction Angle', 'Friction Angle', 'friction_angle']
             if c in df.columns), None)
        if fa_col:
        ...
        # Plasticity index
        pi_col = next(
            (c for c in ['Plasticity Index (PI)', 'Plasticity Index', 'plasticity_index', 'PI']
             if c in df.columns), None)
        if pi_col:
        ...
        # Moisture content
        wc_col = next(
            (c for c in ['Natural Water Content (ω)', 'Natural Water Content',
        ...
        # D50
        d50_col = next(
            (c for c in ['Mean Particle Size (D50) (mm)', 'Mean Particle Size',
    ...
    def process_and_validate(self) -> bool:
        ...
        for sheet_name, df in self.raw_data.items():
            ...
            # FIX 4 — null implausibly low PGA
    ...
    def compute_missing_raw_columns(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        es_col = next(
            (c for c in ['Elastic Modulus (Es) (MN/m²)', 'elastic_modulus_es']
             if c in df.columns), None)
        if es_col:
    ...
    def fill_missing_by_borehole(self) -> bool:
        ...
        # FIX 10 — fault-distance attenuation fallback
        fault_col = next(
            (c for c in ['Fault Distance (km)', 'fault_distance'] if c in df.columns), None)
        if fault_col:
    ...
    def calculate_csr_crr(self) -> bool:
        ...
        print(f"    N1(60) source — measured: {has_measured_n160.sum()}, "
              f"Cn-computed: {(~has_measured_n160).sum()}")

        # NCEER fines correction
    ...
    # -----------------------------------------------------------------------
    # Bearing capacity — initial pass with fixed dims (Bowles 1988)
    # -----------------------------------------------------------------------
    def calculate_bearing_bowles(self) -> bool:
        """
        Initial bearing capacity using fixed B=3.0m, D=1.5m.
        Results are OVERWRITTEN by recalculate_bearing_with_ann_foundation()
        which uses per-layer ANN-derived B and D.
        This step is kept to ensure bearing_qa_kpa exists in processed_data
        before the classify step reads q_actual_kpa.
        """
        print("\n" + "=" * 80)
    ...
    # -----------------------------------------------------------------------
    # FIX 12 — Bearing capacity recalc with ANN foundation dimensions
    # -----------------------------------------------------------------------
    def recalculate_bearing_with_ann_foundation(self) -> bool:
        """
        FIX 12 — Recompute bearing_qa_kpa, bearing_qu_kpa, and settlement_mm
        using per-layer ANN-consistent foundation dimensions B and D
        (mirrors compute_B_target / compute_D_target from the ANN trainer v5).

        Previously the pipeline used hardcoded B=3.0m, D=1.5m for every layer,
        which inflated bearing capacity (e.g. 1076 kPa for a typical SM layer)
        ...
        After this step:
          ...
          - settlement_mm   : corrected settlement estimate
        """
        print("\n" + "=" * 80)
        ...
        def _B(row):
            n = row.get('spt_n160') if pd.notna(
                row.get('spt_n160')) else row.get('spt_n_value', 15.0)
            fc = row.get('fines_content',      15.0)
            gwl = row.get('groundwater_depth_m', 5.0)
            try:
                return _compute_foundation_B(float(n), float(fc), float(gwl))
            except Exception:
                return 2.0
        ...
        def _D(row):
            ...
            fc = row.get('fines_content',       15.0)
            gwl = row.get('groundwater_depth_m',  5.0)
            uscs = row.get('uscs_symbol',          '')
            risk = row.get('liquefaction_risk_level', '')
            try:
                return _compute_foundation_D(float(n), float(fc), float(gwl), uscs, risk)
            except Exception:
                return 1.5
        ...
        df['foundation_kd'] = Kd.round(4)
        df['bearing_qa_kpa'] = Qa.round(2)
        df['bearing_qu_kpa'] = (Qa * 3.0).round(2)
        df['settlement_mm'] = ((df['q_actual_kpa'] / Qa) * SI).round(2)

        print(f"  B range       : {B.min():.2f} – {B.max():.2f} m")
        print(f"  D range       : {D.min():.2f} – {D.max():.2f} m")
        print(f"  Qa range      : {Qa.min():.1f} – {Qa.max():.1f} kPa")
        print(
            f"  Settlement    : {df['settlement_mm'].min():.1f} – {df['settlement_mm'].max():.1f} mm")
    ...
    def engineer_features(self) -> bool:
        ...
        df['effective_stress_ratio'] = (
            df['effective_overburden_pressure'] / (df['total_overburden_pressure'] + 1))
        ...
        df['cn_factor'] = np.minimum(
            1.7, np.sqrt(101.3 / df['effective_overburden_pressure'].clip(1.0)))
    ...
    def export_csv(self) -> bool:
        ...
        output_columns = [
            ...
            'spt_n_value', 'spt_n_value_liq', 'spt_n160',
            'spt_is_refusal', 'spt_is_imputed',
    ...
    def store_soil_layers(self) -> bool:
        """
        FIX 11 — All schema columns now populated:
        ...
        Run this SQL before first pipeline run if columns are missing:
          ...
          ALTER TABLE soil_layers
            ...
            ADD COLUMN IF NOT EXISTS liquefaction_probability FLOAT,
            ADD COLUMN IF NOT EXISTS liquefaction_status      VARCHAR,
        ...
        for _, row in df.iterrows():
            ...
            record = {
                ...
                # ── FIX 11 — previously null, now populated ───────────────
                ...
                # ── Remarks ───────────────────────────────────────────────
                'remarks': None,   # all data now in proper columns
            }
...
""")
    ...
    def run(self) -> bool:
        ...
        steps = [
            ...
            ("Recalc Bearing with ANN Dims",
             self.recalculate_bearing_with_ann_foundation),
        ...
        print(f"  Fixes applied: FIX 1–12")
        return True
