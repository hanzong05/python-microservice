...
async def get_data_statistics():
    ...
    try:
        ...
        return DataStats(
            ...
            depth_range={"min": float(
                result[4] or 0), "max": float(result[5] or 0)}
        )
...
async def prepare_ml_training_data(request: TrainingDataRequest):
    ...
    try:
        ...
        if request.include_features:
            feature_cols = [
                f for f in feature_cols if f in request.include_features]
        ...
        X_train, X_test, y_train, y_test = train_test_split(
            ...
            # Stratify by liquefaction
            stratify=df_clean['liquefaction_binary']
        )
