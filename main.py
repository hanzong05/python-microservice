...
def load_model():
    ...
    try:
        ...
        print(f"  [OK] Scaler loaded (expects {_scaler.n_features_in_} features)")
        
        try:
            multi_data = client.storage.from_(bucket_name).download('models/ann_multi_output.pkl')
        ...
        try:
            liq_data = client.storage.from_(bucket_name).download('models/ann_liquefaction_classifier.pkl')
        ...
        try:
            settle_data = client.storage.from_(bucket_name).download('models/ann_settlement_regressor.pkl')
        ...
        try:
            bearing_data = client.storage.from_(bucket_name).download('models/ann_bearing_capacity_regressor.pkl')
...
def find_nearest_boreholes(latitude: float, longitude: float, n: int = 5, max_distance_km: float = 100) -> List[Tuple[Dict, float]]:
    ...
    try:
        ...
        for borehole in query.data:
            ...
            distance_km = haversine_distance(latitude, longitude, bh_lat, bh_lon)
        ...
        print(f"[INFO] Found {len(nearest)} boreholes within {max_distance_km}km")
...
def get_borehole_soil_data(borehole_id: str, depth_m: Optional[float] = None) -> List[Dict]:
    ...
    try:
        query = client.table('v_complete_soil_data').select('*').eq('borehole_id', borehole_id)
        
        if depth_m:
            query = query.gte('depth_from_m', depth_m - 2.0).lte('depth_to_m', depth_m + 2.0)
...
async def predict(
    ...
    n_boreholes: Optional[int] = Query(5, ge=1, le=10, description="Number of nearest boreholes to use for interpolation")
):
    """
    ...
    try:
        ...
        if multi_model is not None:
            ...
            settlement_cm = float(settle_model.predict(features_scaled)[0]) if settle_model else 0.0
            bearing_capacity_kpa = float(bearing_model.predict(features_scaled)[0]) if bearing_model else 200.0
        ...
        capacity_reduction = ((pre_bearing - post_bearing) / pre_bearing) * 100 if pre_bearing > 0 else 0
